...
namespace StraliSolutions.CSOMSAML.Auth
{
    ...
    public class ClaimsWebAuth : IDisposable
    {
        ...
        public CookieCollection Show()
        {
            ...
            DisplayLoginForm.Visible = false;
        ...
        private bool ExtraHeadersFromResponse(WebResponse response, out string loginUrl, out Uri navigationEndUrl)
        {
            ...
            try
            {
                Uri navUri = new Uri(response.Headers[Constants.CLAIM_HEADER_RETURN_URL]);
                navigationEndUrl = new Uri(navUri.ToString().Replace(navUri.PathAndQuery, String.Empty));
        ...
        private void ClaimsWebBrowser_Navigated(object sender, WebBrowserNavigatedEventArgs e)
        {
            ...
            if (fldNavigationEndUrl != null && e.Url.ToString().StartsWith(fldNavigationEndUrl.ToString(), StringComparison.OrdinalIgnoreCase))
            {
